...
#define DEVICE_SCREEN_X ([UIScreen instancesRespondToSelector:@selector(currentMode)] ? CGSizeEqualToSize(CGSizeMake(1125, 2436), [[UIScreen mainScreen] currentMode].size) : NO)
...
static inline dispatch_source_t gcdTimer(NSTimeInterval delay,NSUInteger times, dispatch_block_t block)
{
    if (block) {
        ...
        dispatch_source_t dispatch_source_timer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0));
        dispatch_source_set_timer(dispatch_source_timer, DISPATCH_TIME_NOW, delay * NSEC_PER_SEC, 0.0);
        dispatch_source_set_event_handler(dispatch_source_timer, ^{
            index++;
            if (times != 0 && index == times) {
                // 取消定时器
                dispatch_cancel(dispatch_source_timer);
            }
            block();
        });
        dispatch_resume(dispatch_source_timer);
        return dispatch_source_timer;
    } else {
        return nil;
    }
}
